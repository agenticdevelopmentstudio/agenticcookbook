...
Standalone cookbook directories MUST use the suffix `-cookbook`. For a source repository named `my-app`, the cookbook directory is `my-app-cookbook/`. This distinguishes cookbooks from other directories and makes the project type immediately recognizable.

A cookbook that specifies code in its own repository (a library cookbook, `structure.kind: library`) is instead that repository's `cookbook/` directory. Its directory tree mirrors the arrangement of the code, each spec is named by its file name alone, and each spec lists its implementations in `## Reference Implementations`:

```
my-repo/
├── cookbook/
│   ├── cookbook.json          # structure.kind: library, code.roots
│   └── ai-plugin-kit/
│       ├── ai-plugin.md
│       └── models/
│           └── model-capability.md
└── packages/...               # the code the specs point at
```
